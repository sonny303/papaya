...
for (const name of ["couple-planning", "planning-board"]) {
  for (const baseline of expected) {
    const dimensions = {
      width: baseline.width,
      height:
        name === "planning-board"
          ? Math.round((baseline.width * 965) / 1448)
          : baseline.height,
    };
    for (const format of ["avif", "webp"]) {
      const path = join(
        root,
        `public/assets/${name}-${dimensions.width}.${format}`,
      );
      try {
        await access(path);
...
console.log(`Responsive asset check passed (${expected.length * 4} files).`);
